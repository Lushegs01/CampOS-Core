...
import { Calendar, ArrowUpRight, ArrowDownRight, TrendingUp, Users, GraduationCap, Building2, Hotel, MessageCircle, Loader2 } from "lucide-react";
...
export default function AnalyticsPage() {
  ...
  const dailyData = analyticsData?.daily || mockDailyData;
  const moduleBreakdown = analyticsData?.modules || mockModuleBreakdown;
  const summary = analyticsData?.summary;

  const stats = summary ? [
    { label: "Total Students", value: summary.totalStudents.toLocaleString(), change: summary.totalStudentsChange, icon: Users, color: "bg-emerald-500" },
    { label: "Avg Attendance", value: `${summary.attendanceRate}%`, change: summary.attendanceRateChange, icon: TrendingUp, color: "bg-blue-500" },
    { label: "Housing Occupancy", value: `${summary.housingOccupancy}%`, change: summary.housingOccupancyChange, icon: Hotel, color: "bg-amber-500" },
    { label: "NADA Engagement", value: `${(summary.nadaActiveUsers / 1000).toFixed(1)}K`, change: summary.newRegistrationsChange, icon: MessageCircle, color: "bg-violet-500" },
  ] : [
